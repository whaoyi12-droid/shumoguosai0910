"""Prepare official Q3 six-hour refund export only from 334 complete, validated, linked real-data days."""
import argparse
import json
...
from solve_question3_six_hour import verify_ledger


def prepare(folder, base=ROOT):
    metadata=json.loads((folder/'metadata.json').read_text(encoding='utf-8'))
    if metadata.get('model') not in ('Q3-six-hour-24h-MILP-refund-v2','Q3-six-hour-24h-MILP-refund-v3'):
        raise ValueError('This exporter requires the six-hour refund model')
